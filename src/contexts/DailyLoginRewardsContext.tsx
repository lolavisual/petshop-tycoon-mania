...
interface DailyLoginRewardsContextValue {
  rewards: DailyReward[];
  userProgress: UserLoginReward | null;
  loading: boolean;
  claiming: boolean;
  canClaimToday: () => boolean;
  claimReward: () => Promise<DailyReward | null>;
  refresh: () => Promise<void>;
}

const DailyLoginRewardsContext = createContext<DailyLoginRewardsContextValue | null>(null);
...
export const isDismissedToday = (): boolean => {
  try {
    return localStorage.getItem(DAILY_REWARDS_DISMISSED_KEY) === getTodayKey();
  } catch {
    return false;
  }
};

export const setDismissedToday = (): void => {
  try {
...
interface DailyLoginRewardsProviderProps {
  children: ReactNode;
}

export function DailyLoginRewardsProvider({ children }: DailyLoginRewardsProviderProps) {
  ...
  return (
    <DailyLoginRewardsContext.Provider
      value={{
        rewards,
        userProgress,
        loading,
...
export function useDailyLoginRewardsContext() {
  const context = useContext(DailyLoginRewardsContext);
  if (!context) {
    throw new Error('useDailyLoginRewardsContext must be used within DailyLoginRewardsProvider');
  }
  return context;
}
